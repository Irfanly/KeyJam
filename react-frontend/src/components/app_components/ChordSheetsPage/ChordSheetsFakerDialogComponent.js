...
import chordSheetsFakerFactory from "./chordSheetsFakerFactory";
...
const ChordSheetsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = chordSheetsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("chordSheets").create(elem));
...
export default connect(mapState, mapDispatch)(ChordSheetsFakerDialogComponent);
