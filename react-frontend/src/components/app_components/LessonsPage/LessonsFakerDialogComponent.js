...
import lessonsFakerFactory from "./lessonsFakerFactory";
...
const LessonsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = lessonsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lessons").create(elem));
...
export default connect(mapState, mapDispatch)(LessonsFakerDialogComponent);
