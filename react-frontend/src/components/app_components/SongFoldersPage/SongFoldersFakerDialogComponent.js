...
import songFoldersFakerFactory from "./songFoldersFakerFactory";
...
const SongFoldersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = songFoldersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("songFolders").create(elem));
...
export default connect(mapState, mapDispatch)(SongFoldersFakerDialogComponent);
